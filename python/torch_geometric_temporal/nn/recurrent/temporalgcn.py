...
from torch_geometric_temporal.nn.conv.gcn_conv import GCNConv
...
class TGCN(torch.nn.Module):
    ...
    def _set_hidden_state(self, X, H, target_num):
        if H is None:
            H = torch.zeros(target_num, self.out_channels).to(X.device)
        return H

    def _calculate_update_gate(self, X, edge_index, edge_weight, H, deg):
        Z = torch.cat([self.conv_z(X, edge_index, edge_weight, deg)[0:len(H),:], H], axis=1)
        Z = self.linear_z(Z)
        Z = torch.sigmoid(Z)
        return Z

    def _calculate_reset_gate(self, X, edge_index, edge_weight, H, deg):
        R = torch.cat([self.conv_r(X, edge_index, edge_weight, deg)[0:len(H),:], H], axis=1)
        R = self.linear_r(R)
        R = torch.sigmoid(R)
        return R

    def _calculate_candidate_state(self, X, edge_index, edge_weight, H, R, deg):
        H_tilde = torch.cat([self.conv_h(X, edge_index, edge_weight, deg)[0:len(H),:], H * R], axis=1)
